...
json_file_path = 'C:/Users/5/Desktop/map_visualization/가로수길/시각화_전국가로수길정보표준데이터_name(True).json'
...
with open(sql_file_path, 'r', encoding='utf-8') as file:
    ...
    for line in lines:
        ...
        if match:
            ...
            # 필요한 열 추출 (위도 및 경도와 가로수길 이름)
            if len(values) >= 6:  # 필요한 값이 있는지 확인
                r_na = values[1].strip("'")  # 가로수길 이름
