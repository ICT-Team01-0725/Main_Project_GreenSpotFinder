...
<script src="resources/js/finalroad.js"></script>
</head>
...
<body>
							...
							<li>
								<figure class="category_icon">
									<img src="resources/images/garosu_category_icon01.png"
										alt="벚나무">
								</figure> <span class="category"> <a href="/roads?type=벚">벚나무</a>
							</span>
							</li>
							<li>
								<figure class="category_icon">
				...
				<ul class="paging_num_ul">
					...
					<li
						...
						<c:if test="${currentPage > 1}">
							<a href="/roads?type=${type}&page=${currentPage - 1}">chevron_left</a>
						</c:if>
					</li>
	...
	<script>
		...
		var roads = JSON.parse('${roadsJson}'); // 서버에서 전달받은 JSON 데이터를 JavaScript 객체로 변환

		// 마커를 표시할 공원의 위도, 경도 값으로 반복하여 마커를 찍는다
		var markers = [];
		roads.forEach(function(road) {
			if (road.r_s_lat && road.r_s_lon) {
				var markerPosition = new kakao.maps.LatLng(road.r_s_lat, road.r_s_lon); // 마커의 위치
